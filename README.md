...
현재는 **재구축 단계 2**로, 승인된 AABB 레이 마칭 위에 다음 기능을 검증합니다.
...
- ImGui Noise Lab의 XY/XZ/YZ 동기 단면과 실시간 파라미터 조절
...
현재는 절차적 value noise 하나와 고정 산란색만 사용합니다. 높이 프로파일, detail noise, weather map, 태양광과 early exit는 이후 단계까지 의도적으로 구현하지 않습니다.

Noise Lab은 실제 구름 위에 떠 있는 개발 창입니다. 세 단면의 교차점을 클릭하거나 `Slice XYZ`를 움직여 3D 공간을 탐색하고, Raw/Threshold/Final 출력을 전환할 수 있습니다. `Export 3 PNG + JSON`은 관찰 자료만 저장하며 실제 구름은 PNG가 아니라 같은 절차적 3D 함수를 계속 사용합니다.
...
`Foundation*`과 `Stage1*`은 이전 단계 회귀를 검사합니다. `Stage2NoiseMath`는 noise 결정성·범위·coverage·바람 좌표를, `Stage2Smoke`는 단계 2 디버그와 프리셋의 실제 D3D11 경로를 검사합니다. `NoiseLabSmoke`는 세 출력과 PNG/JSON을, `ShaderHotReloadSmoke`는 공용 include 변경·컴파일 실패·복구 시 구름과 단면의 원자적 교체를 검사합니다.
