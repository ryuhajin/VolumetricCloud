...
- 코드를 바꾸는 모든 브랜치는 `changes/` 아래에 브랜치 작업 기록을 하나 둡니다.
  ...
  - 에이전트는 렌더 캡처를 생성하거나 자동 비교하지 않습니다. 반환 코드, 수치 범위, HLSL 컴파일, D3D11 디버그 메시지, 빌드와 테스트 결과를 기록합니다.
  - 화면 품질은 자동 승인하지 않습니다. 날짜, 프리셋, 카메라, 사용자의 관찰 결과를 기록하고 사용자가 최종 승인합니다.
  - 필수 항목은 목표와 배경, 기존 문제, 근거, 대안, 해결 방법, 실제 변경, 영향, 검증, 후속 과제입니다.
- 개인 면접 답변과 회고 문구는 공개 작업 기록과 분리해 `notes/interview/`에 둡니다.
  - 이 폴더는 `.gitignore` 대상이며 커밋하거나 PR에 첨부하지 않습니다.
  - 필요하면 브랜치별로 `<브랜치명>.md`를 만들고 문제 상황, 판단, 구현, 검증, 배운 점을 정리합니다.
  - 기술적 사실의 원본은 추적되는 `changes/` 문서이며, 로컬 면접 메모에는 이를 개인적인 답변 형태로만 재구성합니다.
