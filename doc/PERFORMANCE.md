...
Stage 13 품질 재검증은 50km 구간에서 100m 목표 간격을 유지하도록 View 상한을 512로 둔다.
이 값의 GPU Cloud 비용은 사용자 화면 승인용 참고값으로만 기록하며, 성능 회복과 256/512 비교는
단계 13 승인 뒤 재개하는 단계 9의 새 평면층 기준에서 판정한다.

`Stage13Smoke`는 F6, Raw Noise, Optimization Off, Wind 0, time 0을 고정하고 Base
`0.00035 cycle/m`와 고주파 스트레스 `0.00662 cycle/m`를 각각 8프레임 렌더링한다.
각 조건의 Cloud Pass 백버퍼 해시가 프레임마다 같아야 하며 두 스케일의 해시는 서로 달라야 한다.
이 검사는 성능이나 미적 품질이 아니라 고정 입력에 대한 GPU 출력 결정성만 판정한다.
