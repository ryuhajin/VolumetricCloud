...
beauty 모드는 가로·세로 0.5배의 `R11G11B10_FLOAT` color와 `R16_FLOAT` 첫 유효 구름 거리 MRT에서 raymarch한다. 4-frame jitter로 서로 다른 서브픽셀 위치를 평가한 뒤 full-resolution history color/depth 두 세트를 ping-pong한다. resolve는 jitter NDC를 UV 오프셋으로 역변환해 현재 color/depth를 unjittered 출력 위치에 정렬하므로 toggle이나 history reset 뒤에도 화면 전체가 서브픽셀만큼 흔들리지 않는다.
