...
> Stage 3 / `feature/cloud-quality` 기준.

## 현재 구현

| 영역 | 상태 |
|---|---|
| 레이마칭 | AABB 교차, 48~128 view step, 고정 jitter, 빈 공간 2배 진행 |
...
| 캐시 | v4 `.cso` + 128³ RGBA8 base + 64³ RGBA8 detail |
| 진단 | 16개 렌더 모드, 4-MRT Noise Inspector, 프리셋과 캐시 작업 |
| 검증 | seam, RGBA 분산, 밀도 비퇴화, cache round-trip, 산란 수치 |
...
| 슬롯 | 리소스 | 내용 |
|---|---|---|
| b0 | `CameraCB` | 역 view-projection, 카메라, AABB |
| b1 | `CloudCB` | 128바이트 노이즈·형태·조명·샘플링 파라미터 |
...
ImGui Begin
  ...
  → [생성 파라미터 변경 시] base/detail compute dispatch
...
정상 v4 캐시 시작에서는 HLSL runtime compile과 noise dispatch가 모두 0회다.

## F1 진단

- 형태: Final Density, Base Shape, Detail, Height, Base R/G/B/A
...
1. 사용자 시각 승인과 Showcase 값 미세 조정
2. GPU timestamp query로 실제 GPU frame time 측정
3. 장면 depth 합성
4. weather map과 넓은 구름층
5. 반해상도 temporal reprojection

변경 이유와 대안은 [feature-cloud-quality 변경 기록](changes/feature-cloud-quality.md)을 참고한다.
