...
> 현재 상태(2026-09-01): 아래 단계별 내용은 구현 당시의 설계·승인 이력이다. 최종 런타임은
> Stage 15 High 단일화에 따라 `9 → 12 → 14 → 15`의 한 경로만 사용한다. Stage 10 저해상도
> 복원과 Stage 11 Temporal은 비교 완료 뒤 폐기했으며, Cirrus/Desert, Detail LOD, NTE Rim,
> Capture/Reference와 런타임 최적화 프리셋도 제거했다. 현재 규범은
> [ROADMAP](ROADMAP.md), [ARCHITECTURE](ARCHITECTURE.md),
> [Stage 15 검증 가이드](STAGE15_PRESET_VALIDATION_GUIDE.md)다.
...
Stage 15 최종 간소화의 화면 승인은 아직 완료되지 않았다. 과거 단계 승인은 이력으로
유효하지만 폐기된 기능을 현재 런타임으로 복구하라는 의미는 아니다.
