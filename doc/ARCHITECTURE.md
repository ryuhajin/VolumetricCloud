...
현재는 재구축 단계 6이다. Weather 밀도장에서 태양 방향 광학 깊이와 직접 단일 산란을 계산한다.
...
| `LightParameters` | 48바이트 태양 방향·색·세기와 Light Ray 품질 설정 |
...
| `LightParameters.hlsli` / `CloudLighting.hlsli` | 공유 48바이트 조명 설정과 Base-only Light Ray |
...
9. 태양 투과율과 현재 View 투과율로 직접 단일 산란을 누적한다.
10. Noise Lab이 같은 CloudCB·Weather SRV·시간으로 단면과 조명 UI를 갱신하고 성능 오버레이를 그린다.
11. GPU Frame timestamp를 닫은 뒤 VSync 설정에 따라 `Present(1, 0)` 또는 `Present(0, 0)`을 호출한다.
...
### `LightParameters` / `LightCB` (`b3`, 48바이트)
...
| 2 | `maxLightSteps`, `lightStepSize`, `lightRayBias`, `lightPadding` | `16`, `0.25m`, `0.01m`, `0`; 품질 상한·목표 간격·시작 bias·정렬 |

LightCB는 CloudCB와 분리해 `b3`에 바인딩한다. 방향은 빛의 진행 방향이 아니라
현재 표본에서 태양으로 나가는 방향이다. CPU는 방향을 정규화하고 음수·비정상
값을 안전 범위로 제한한다. Light Ray는 `EvaluateBaseCloudDensity`만 호출한다.
...
`weather-map.png`, 모든 생성 설정·맵 해시·LightCB와 태양 프리셋을 담은 schema 6 JSON을 기록한다.
...
parameters`, `Height profile`, `Detail erosion`, `Performance`, `Directional light`, `Animation` 대분류는 독립적으로
...
- 외부 Weather PNG 로딩·페인팅·precipitation, fBm/Worley, phase function과 shadow
