...
현재는 재구축 단계 3다. 승인된 월드 공간 3D noise 밀도에 AABB의 월드 Y 기반 상·하단 높이 프로파일을 곱한다.
...
| `CloudParameters` | 96바이트 AABB·noise·바람·높이·step 설정과 디버그 모드 |
| `DiagnosticScene.hlsl` | 평면·박스의 불투명 색상과 장치 깊이 출력 |
| `Ray.hlsli` | 평행축 0 나누기를 피하는 slab Ray-AABB 교차 |
| `CloudParameters.hlsli` / `Noise.hlsli` | 공유 96바이트 CloudCB와 3D noise·높이 밀도 함수 |
...
6. coverage 기본 밀도에 상·하단 높이 프로파일과 density multiplier를 곱해 적분한다.
7. Noise Lab이 같은 `CloudCB`와 유효 시간을 사용해 noise·높이 XY/XZ/YZ 단면을 갱신한다.
...
구조체는 16바이트 묶음 여섯 개다. `transmittanceThreshold`는 단계 9 early exit 전까지 읽지 않고 `heightProfilePadding`은 GPU 정렬에만 사용한다.

### `NoiseLabParameters` / `NoiseLabCB` (`b2`, 32바이트)

| 묶음 | 필드 | 의미 |
|---|---|---|
| 0 | `normalizedSlicePosition(float3)`, `outputMode` | AABB 내부 0~1 교차점과 raw/threshold/final/height 선택 |
...
- detail noise, weather map, 태양광, phase function과 shadow
