...
학습 프로젝트입니다. 현재는 **재구축 단계 0**을 진행하며, 불투명 진단 장면에서 월드 레이,
Scene Depth 복원과 별도 합성 패스를 검증합니다.
...
- **실행:** `build/Debug/VolumetricCloud.exe` (드래그/휠, 숫자 0~4 디버그, F5~F7 카메라)
...
| PS | `shaders/VolumetricClouds.hlsl` | 레이·깊이 복원과 단계 0 합성 |
...
4. 현재 단계 0 범위 밖(AABB, density, noise, light, temporal)은 다음 단계로 분리합니다.
