...
평면 구름층 실험은 별도 브랜치에 보관했으며, 현재는 단계 9 이후 순서·범위·성능 기준을
다시 계획하는 중입니다. 새 계획이 승인되기 전에는 후속 기능 구현을 시작하지 않습니다.
...
4. 현재 재계획 기간에는 `src/`, `shaders/`, `CMakeLists.txt`를 단계 8 태그와 동일하게 유지합니다.
   단계 9 이후 최적화·저해상도·temporal·Cloud Shadow·평면/구형 구름층은 새 계획 승인 전 구현하지 않습니다.
