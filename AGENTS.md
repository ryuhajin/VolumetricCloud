...
학습 프로젝트입니다. 단계 9 최적화는 소규모 AABB `DenseExterior`에서 성능 기준을
통과하지 못해 보류했습니다. 현재는 **재구축 단계 13을 선행**하며 유한 AABB를
인디 오픈월드용 1.5~4.5km 평면 구름층으로 교체합니다.

현재 실행 순서는 `0~8 완료 → 13 선행 → 9 재개 → 10 → 11 → 12 → 14 → 15`입니다.
단계 13 사용자 승인 전에는 단계 9 성능 게이트를 다시 판정하지 않습니다.
현재 단계 13 기준 광학값은 extinction `0.00075/m`, density `0.65`, 단일산란 알베도
`0.90`이며 Detail은 8~20km에서 평균 필터 후 20km 밖에서 호출을 생략합니다.
...
- **실행:** `build/Debug/VolumetricCloud.exe` (Shift 조명·Phase, Ctrl+J 누적 직접광 진단)
...
| Weather Map | `src/WeatherMap.*` | 256² CPU RGBA 프리셋 생성과 해시 |
...
| 평면층 기준 | `src/Stage13CloudLayerMath.h` | 구름층 교차·거리 fade·주기 Weather 좌표 기준 |
...
4. 현재 단계 13에서는 단계 9 재튜닝, 저해상도, temporal, Cloud Shadow, 구형 행성과 대기 통합을 구현하지 않습니다.
