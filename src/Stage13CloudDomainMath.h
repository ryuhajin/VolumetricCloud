// ============================================================================
//  Stage13CloudDomainMath.h - 단계 13-1 Y 평면층 교차 CPU 기준
// ============================================================================
#pragma once
...
namespace stage13domain
{
struct CloudInterval
{
    bool hit = false;
    double start = 0.0;
    double end = 0.0;
...
inline CloudInterval IntersectPlanarLayer(
    double originY, double directionY, double bottomAltitude,
    double layerThickness, double sceneDistance,
    double maximumTraceDistance)
{
    CloudInterval result;
    const bool finite = std::isfinite(originY) &&
        std::isfinite(directionY) && std::isfinite(bottomAltitude) &&
        std::isfinite(layerThickness) && std::isfinite(sceneDistance) &&
        std::isfinite(maximumTraceDistance);
