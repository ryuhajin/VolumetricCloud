...
private:
    // HLSL 파일을 런타임 컴파일 (실패 시 메시지박스 + false)
    ...
    struct CameraCB
    {
        ...
        float               time;         //  4
        DirectX::XMFLOAT3   sphereCenter; // 12
        float               sphereRadius; //  4
        DirectX::XMFLOAT2   screenSize;   //  8
    ...
    std::wstring m_shaderDir; // 실행 파일 옆 shaders/ 경로
};
